 ...
 * 2_dragdrop.js
 * 3_editor.js
 * 3_editor_delimiter.js
 * 3_editor_header.js
 * 3_editor_imageTool.js
 * 3_editor_list.js
 * 3_editor_table.js
 * scripts.js
 * Generated: 2023-01-03 05:33:34
 */


/* Filename: 0_interact.js */
...
default:return event}},isMultipleTouches=function(event){return(event.targetTouches||event.target.touches||[]).length>1},getTimestamp=function(){return new Date().getTime()},sendEvent=function(elm,eventName,originalEvent,data){var customEvent=doc.createEvent('Event')
customEvent.originalEvent=originalEvent
data=data||{}
data.x=currX
data.y=currY
if(defaults.useJquery){customEvent=jQuery.Event(eventName,{originalEvent:originalEvent})
jQuery(elm).trigger(customEvent,data)}
if(customEvent.initEvent){for(var key in data){customEvent[key]=data[key]}
customEvent.initEvent(eventName,true,true)
elm.dispatchEvent(customEvent)}
while(elm){if(elm['on'+eventName])
elm['on'+eventName](customEvent)
elm=elm.parentNode}},onTouchStart=function(e){if(!isTheSameFingerId(e)||isMultipleTouches(e))return
...
target=e.target},defaults.longtapThreshold)
timestamp=getTimestamp()
tapNum++},onTouchEnd=function(e){if(!isTheSameFingerId(e)||isMultipleTouches(e))return
pointerId=undefined
if(e.type==='mouseup'&&wasTouch){wasTouch=false
return}
...
tapNum=0}else{if(cachedX>=currX-defaults.tapPrecision&&cachedX<=currX+defaults.tapPrecision&&cachedY>=currY-defaults.tapPrecision&&cachedY<=currY+defaults.tapPrecision){if(timestamp+defaults.tapThreshold-now>=0){sendEvent(e.target,tapNum>=2&&target===e.target?'dbltap':'tap',e)
target=e.target}}
dblTapTimer=setTimeout(function(){tapNum=0},defaults.dbltapThreshold)}},onTouchMove=function(e){if(!isTheSameFingerId(e))return
if(e.type==='mousemove'&&wasTouch)return
var pointer=getPointerEvent(e)
currX=pointer.pageX
currY=pointer.pageY},tapNum=0,pointerId,currX,currY,cachedX,cachedY,timestamp,target,dblTapTimer,longtapTimer
setListener(doc,touchevents.touchstart+(defaults.justTouchEvents?'':' mousedown'),onTouchStart)
setListener(doc,touchevents.touchend+(defaults.justTouchEvents?'':' mouseup'),onTouchEnd)
setListener(doc,touchevents.touchmove+(defaults.justTouchEvents?'':' mousemove'),onTouchMove)
win.tocca=function(options){for(var opt in options){defaults[opt]=options[opt]}
return defaults}})(document,window)

/* Filename: 1_mike.js */
function serialize(form){if(!form||form.nodeName!=="FORM"){return;}
var i,j,q=[];for(i=form.elements.length-1;i>=0;i=i-1){if(form.elements[i].name===""){continue;}
switch(form.elements[i].nodeName){case"INPUT":switch(form.elements[i].type){case"text":case"email":case"search":case"hidden":case"password":case"button":case"reset":case"color":case"date":case"datetime-local":case"submit":q.push(form.elements[i].name+"="+
encodeURIComponent(form.elements[i].value));break;case"checkbox":case"radio":if(form.elements[i].checked){q.push(form.elements[i].name+"="+
encodeURIComponent(form.elements[i].value));}
break;case"file":break;}
break;case"TEXTAREA":q.push(form.elements[i].name+"="+
encodeURIComponent(form.elements[i].value));break;case"SELECT":switch(form.elements[i].type){case"select-one":q.push(form.elements[i].name+"="+
encodeURIComponent(form.elements[i].value));break;case"select-multiple":for(j=form.elements[i].options.length-1;j>=0;j=j-1){if(form.elements[i].options[j].selected){q.push(form.elements[i].name+"="+
encodeURIComponent(form.elements[i].options[j].value));}}
break;}
break;case"BUTTON":switch(form.elements[i].type){case"reset":case"submit":case"button":q.push(form.elements[i].name+"="+
encodeURIComponent(form.elements[i].value));break;}
break;}}
return q.join("&");}
function easeOut(progress,power=2){return 1-Math.pow(1-progress,power);}
function easeInOutCirc(x){return x<0.5?(1-Math.sqrt(1-Math.pow(2*x,2)))/ 2:(Math.sqrt(1-Math.pow(-2*x+2,2))+1)/ 2;}
function easeInOutBack(x){var c1=1.70158;var c2=c1*1.525;return x<0.5?(Math.pow(2*x,2)*((c2+1)*2*x-c2))/ 2:(Math.pow(2*x-2,2)*((c2+1)*(x*2-2)+c2)+2)/ 2;}
function easeInBack(x){var c1=1.70158;var c3=c1+1;return c3*x*x*x-c1*x*x;}
function tween({from=0,to=1,duration=300,ease=easeOut,onUpdate,onComplete,}={}){const delta=to-from;const startTime=performance.now();function update(currentTime){const elapsed=currentTime-startTime;const progress=Math.min(elapsed / duration,1);const latest=from+ease(progress)*delta;if(onUpdate){onUpdate(latest);}
if(progress<1){requestAnimationFrame(update);}else{if(onComplete){onComplete();}}}
requestAnimationFrame(update);}
function getElementY(query,selector=false,bottom=false){var ele=selector?query:document.querySelector(query);return(window.pageYOffset+
(bottom?ele.getBoundingClientRect().bottom:ele.getBoundingClientRect().top));}
function debounce(func,wait,immediate){var timeout;return function(){var context=this,args=arguments;clearTimeout(timeout);timeout=setTimeout(function(){timeout=null;if(!immediate)func.apply(context,args);},wait);if(immediate&&!timeout)func.apply(context,args);};}
function trackPos(elements,output){forEach(elements,function(index,value){output[index]={top:getElementY(elements[index],true),bot:getElementY(elements[index],true,true),};});}
function markPos(trackPos,playstate=[],switchratio,output){var trackers=Object.entries(trackPos);forEach(trackers,function(index,value){var q=Math.round((1 / value[1].bot)*latestKnownScrollY*100)/ 100;rootCSS("--s"+index,q>=0.5?1:0);rootCSS("--t"+index,q>=1?1:q);if(playstate.includes(index)){rootCSS("--s"+index+"ps",q>=switchratio?"playing":"paused");}});}
var w=window.innerWidth||document.documentElement.clientWidth||document.body.clientWidth;var h=window.innerHeight||document.documentElement.clientHeight||document.body.clientHeight;function getRandomInt(min,max){min=Math.ceil(min);max=Math.floor(max);return Math.floor(Math.random()*(max-min+1))+min;}
function queryString(url){var params=[];var parser=document.createElement("a");parser.href=url;var query=parser.search.substring(1);var vars=query.split("&");for(var i=0;i<vars.length;i++){var pair=vars[i].split("=");params[pair[0]]=decodeURIComponent(pair[1]);}
return params;}
function createElementFromHTML(htmlString){var div=document.createElement("div");div.innerHTML=htmlString.trim();return div;}
if(!Element.prototype.matches)
Element.prototype.matches=Element.prototype.msMatchesSelector||Element.prototype.webkitMatchesSelector;if(!Element.prototype.closest)
Element.prototype.closest=function(s){var el=this;if(!document.documentElement.contains(el))return null;do{if(el.matches(s))return el;el=el.parentElement||el.parentNode;}while(el!==null&&el.nodeType==1);return null;};if(!Element.prototype.addEventListeners){Element.prototype.addEventListeners=function(events,fn){events.split(",").forEach(function(event){this.addEventListener(event,fn,false);});};}
function doesExist(el){var element=document.querySelector(el);return typeof element!="undefined"&&element!=null;}
function doesVarExist(element){return typeof element!="undefined"&&element!=null&&element!="";}
function hasAttr(el,attr){return el.hasAttribute(attr);}
function getAttr(el,attr){return el.getAttribute(attr);}
function forEach(array,callback,scope){for(var i=0;i<array.length;i++){callback.call(scope,i,array[i]);}}
function isChild(obj,parentObj){while(obj!=undefined&&obj!=null&&obj.tagName.toUpperCase()!="BODY"){if(obj==parentObj){return true;}
obj=obj.parentNode;}
return false;}
...
return siblings;};function search(e){console.log(e.target);var input=e.target;var target=document.querySelector(input.getAttribute("data-search"));var results=target.querySelectorAll("[data-result]");var items=results;var text=e.target.value.trim();var pat=new RegExp(text,"i");forEach(items,function(i,item){console.log(item);if(pat.test(item.innerText.trim())){item.classList.remove("d-none");item.removeAttribute("style");}else{item.classList.add("d-none");item.setAttribute("style","display:none !important");}});}
function radioCheckboxSwapper(e){if(hasAttr(e.target,"data-submit")){var closestform=e.target.closest("form");closestform.querySelector('[type="submit"]').click();e.preventDefault;}}
function onSubmit(e){postAndReplace(e);}
function onClick(e){postAndReplace(e);if(e.target.classList.contains("generate")){var password=generate();var target=document.querySelector(e.target.dataset.target);target.value=password;e.preventDefault;}}
function onChange(e){radioCheckboxSwapper(e);}
function onInput(e){if(e.target.type=="color"&&e.target.name=="green"){mixTheme(e.target.value);}
if(e.target.getAttribute("type")=="search"){search(e);}}
function rootCSS(property,value,force=true){if(force){document.documentElement.style.setProperty(property,value);}else{if(compare[property]!=value){compare[property]=value;document.documentElement.style.setProperty(property,value);}else{}}}
function onSwipe(e){const side=document.querySelector("#side");const togg=document.querySelector("#toggleSide");if(e.target==side||isChild(e.target,side)){if(e.type=="swiperight"){togg.checked=true;}
if(e.type=="swipeleft"){togg.checked=false;}}}
document.addEventListener("change",onChange);document.addEventListener("click",onClick);document.addEventListener("input",onInput);document.addEventListener("submit",onSubmit);document.addEventListener("swiperight",onSwipe);document.addEventListener("swipeleft",onSwipe);function fileOnload(e){var $img=document.createElement("img");$img.setAttribute("src",e.target.result);$img.setAttribute("id",$temp);$img.addEventListener("load",function(i){var srcCanvas=document.createElement("canvas"),dstCanvas=document.createElement("canvas");var srcContext=srcCanvas.getContext("2d"),dstContext=dstCanvas.getContext("2d");srcContext.imageSmoothingEnabled=true;dstContext.imageSmoothingEnabled=true;var nseCanvas=document.createElement("canvas"),nseContext=nseCanvas.getContext("2d"),x,y,number,opacity=0.3;nseCanvas.width=80;nseCanvas.height=80;nseContext.fillStyle="#808080";nseContext.fillRect(0,0,80,80);for(x=0;x<nseCanvas.width;x++){for(y=0;y<nseCanvas.height;y++){number=Math.floor(Math.random()*60);nseContext.fillStyle="rgba("+number+","+number+","+number+","+opacity+")";nseContext.fillRect(x,y,1,1);}}
var $w=1280;var $h=720;$portraitToLandscape=i.target.height<i.target.width?i.target.height / i.target.width:i.target.width / i.target.height;srcCanvas.width=$w;srcCanvas.height=srcCanvas.width*$portraitToLandscape;if(i.target.height>i.target.width){srcContext.save();srcContext.translate(srcCanvas.width / 2,srcCanvas.height / 2);srcContext.rotate((90*Math.PI)/ 180);srcContext.drawImage(this,-$h / 2,-$w / 2,srcCanvas.height,srcCanvas.width);srcContext.restore();}else{srcContext.drawImage(this,0,0,srcCanvas.width,srcCanvas.height);}
dstCanvas.width=$w;dstCanvas.height=$h;dstContext.drawImage(srcCanvas,srcCanvas.width / 2-dstCanvas.width / 2,srcCanvas.height / 2-dstCanvas.height / 2,dstCanvas.width,dstCanvas.height,0,0,$w,$h);dstContext.save();dstContext.globalCompositeOperation="overlay";dstContext.filter="blur(.3px)";var ptrn=dstContext.createPattern(nseCanvas,"repeat");dstContext.fillStyle=ptrn;dstContext.fillRect(0,0,dstCanvas.width,dstCanvas.height);dstContext.restore();var $inputAndImgSrc=$temp.hasAttribute("data-upload")?$temp.getAttribute("data-upload"):$temp.closest("input");$inputAndImgSrc=$inputAndImgSrc.split(",");console.log($inputAndImgSrc,"nani?");forEach($inputAndImgSrc,function(i,v){if(document.querySelector(v).tagName=="INPUT"){document.querySelector(v).value=dstCanvas.toDataURL("image/jpeg");}else if(document.querySelector(v).tagName=="IMG"){}});$temp.closest("form").querySelector('[type="submit"]').click();});}
function postAndReplace(e){if(e.target.dataset.push){var etarget=e.target,emethod="method",eaction="data-action";var seriali=serialize(e.target);var method=hasAttr(etarget,emethod)&&doesVarExist(getAttr(etarget,emethod))?getAttr(etarget,emethod):"POST";var url=hasAttr(etarget,eaction)&&doesVarExist(getAttr(etarget,eaction))?getAttr(etarget,eaction):window.location.href;var data;var targets=e.target.dataset.push.split(",");var urls=method=="POST"?url:url;var xhr=new XMLHttpRequest();if(e.target.tagName=="FORM"){var form=etarget;var formData=new FormData(form);console.log(formData);data=formData;}else if(e.target.tagName=="A"){data=e.target.href;}
xhr.open(method,urls,true);xhr.setRequestHeader("X-Requested-With","XMLHttpRequest");xhr.send(data);xhr.onreadystatechange=function(){if(xhr.readyState===4){var response=createElementFromHTML(xhr.response);forEach(targets,function(index,value){var target=document.querySelector(value);var rTarget=response.querySelector(value);if(target&&rTarget){forEach(targets,function(index,value){tween({from:0,to:1,duration:550,onUpdate:(v)=>{target.style.opacity=v;},onComplete:(v)=>{target.style.opacity=null;},});});if(target.tagName=="INPUT"){target.value=rTarget.value;}else{target.innerHTML=rTarget.innerHTML;}}});if(xhr.status=="202"){setTimeout(function(){window.location.href=window.location.href;},2000);}}};e.preventDefault();}}
document.querySelectorAll("[data-tiny-editor]").forEach((editor)=>editor.addEventListener("input",(e)=>reportText(e)));function reportText(e){if(e.target.hasAttribute("data-edit")){var target=document.querySelector(e.target.getAttribute("data-edit"));target.value=e.target.innerHTML;}else if(e.target.hasAttribute("data-edit-closest")){var closestTarget=e.target.closest(e.target.getAttribute("data-edit-closest"));console.log(e.target.closest("input"));}}
function reportReady(ele){if(ele.hasAttribute("data-edit")){var target=document.querySelector(ele.getAttribute("data-edit"));target.value=ele.innerHTML;}else if(ele.hasAttribute("data-edit-closest")){var closestTarget=ele.closest(ele.getAttribute("data-edit-closest"));console.log(ele.closest("input"));}}
document.addEventListener("DOMContentLoaded",function(e){console.log("event",e.type);document.querySelectorAll("[data-tiny-editor]").forEach((editor)=>reportReady(editor));});function generate(length=8){function randomItem(array){return array[Math.floor(Math.random()*array.length)];}
var letters=["bcdfghjklmnpqrstvwxz","aeiouy",];var quantity=1,i,j,res="",curKey;for(i=0;i<quantity;i++){curKey=Math.floor(Math.random()*letters.length);for(j=0;j<length;j++){res+=randomItem(letters[curKey]);curKey=(curKey+1)%letters.length;}
res+="\n";}
return res.trim();}

/* Filename: 2_context-menu.js */
!(function(e,t){"object"==typeof exports&&"undefined"!=typeof module?(module.exports=t()):"function"==typeof define&&define.amd?define(t):((e="undefined"!=typeof globalThis?globalThis:e||self).ContextMenu=t());})(this,function(){"use strict";var e,t,n,i;(e=".ContextMenu{display:none;list-style:none;margin:0;max-width:250px;min-width:125px;padding:0;position:absolute;-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none}.ContextMenu--theme-default{background-color:#fff;border:1px solid rgba(0,0,0,.2);-webkit-box-shadow:0 2px 5px rgba(0,0,0,.15);box-shadow:0 2px 5px rgba(0,0,0,.15);font-size:13px;outline:0;padding:2px 0}.ContextMenu--theme-default .ContextMenu-item{padding:6px 12px}.ContextMenu--theme-default .ContextMenu-item:focus,.ContextMenu--theme-default .ContextMenu-item:hover{background-color:rgba(0,0,0,.05)}.ContextMenu--theme-default .ContextMenu-item:focus{outline:0}.ContextMenu--theme-default .ContextMenu-divider{background-color:rgba(0,0,0,.15)}.ContextMenu.is-open{display:block}.ContextMenu-item{cursor:pointer;display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.ContextMenu-divider{height:1px;margin:4px 0}"),(i=(t=void 0===t?{}:t).insertAt),e&&"undefined"!=typeof document&&((n=document.head||document.getElementsByTagName("head")[0]),((t=document.createElement("style")).type="text/css"),"top"===i&&n.firstChild?n.insertBefore(t,n.firstChild):n.appendChild(t),t.styleSheet?(t.styleSheet.cssText=e):t.appendChild(document.createTextNode(e)));var o=[],s=0;function u(e,t,n){void 0===n&&(n={});var i=document.createEvent("Event");Object.keys(n).forEach(function(e){i[e]=n[e];}),i.initEvent(t,!0,!0),e.dispatchEvent(i);}
Element.prototype.matches||(Element.prototype.matches=Element.prototype.msMatchesSelector);function a(e,t,n){void 0===n&&(n={className:"",minimalStyling:!1}),(this.selector=e),(this.items=t),(this.options=n),(this.id=s++),(this.target=null),this.create(),o.push(this);}
return((a.prototype.create=function(){var i=this;(this.menu=document.createElement("ul")),(this.menu.className="ContextMenu"),this.menu.setAttribute("data-contextmenu",this.id),this.menu.setAttribute("tabindex",-1),this.menu.addEventListener("keyup",function(e){switch(e.which){case 38:i.moveFocus(-1);break;case 40:i.moveFocus(1);break;case 27:i.hide();}}),this.options.minimalStyling||this.menu.classList.add("ContextMenu--theme-default"),this.options.className&&this.options.className.split(" ").forEach(function(e){return i.menu.classList.add(e);}),this.items.forEach(function(e,t){var n=document.createElement("li");"name"in e?((n.className="ContextMenu-item"),(n.textContent=e.name),n.setAttribute("data-contextmenuitem",t),n.setAttribute("tabindex",0),n.addEventListener("click",i.select.bind(i,n)),n.addEventListener("keyup",function(e){13===e.which&&i.select(n);})):(n.className="ContextMenu-divider"),i.menu.appendChild(n);}),document.body.appendChild(this.menu),u(this.menu,"created");}),(a.prototype.show=function(e){(this.menu.style.left=e.x+"px"),(this.menu.style.top=e.y+"px"),this.menu.classList.add("is-open"),(this.target=e.target),this.menu.focus(),e.preventDefault(),u(this.menu,"shown");}),(a.prototype.hide=function(){this.menu.classList.remove("is-open"),(this.target=null),u(this.menu,"hidden");}),(a.prototype.select=function(e){e=e.getAttribute("data-contextmenuitem");this.items[e]&&this.items[e].fn(this.target),this.hide(),u(this.menu,"itemselected");}),(a.prototype.moveFocus=function(e){void 0===e&&(e=1);var t,n=this.menu.querySelector("[data-contextmenuitem]:focus");(t=(t=n?(function e(t,n,i){t=0<(i=void 0===i?1:i)?t.nextElementSibling:t.previousElementSibling;return!t||t.matches(n)?t:e(t,n,i);})(n,"[data-contextmenuitem]",e):t)||(0<e?this.menu.querySelector("[data-contextmenuitem]:first-child"):this.menu.querySelector("[data-contextmenuitem]:last-child")))&&t.focus();}),(a.prototype.on=function(e,t){this.menu.addEventListener(e,t);}),(a.prototype.off=function(e,t){this.menu.removeEventListener(e,t);}),(a.prototype.destroy=function(){this.menu.parentElement.removeChild(this.menu),(this.menu=null),o.splice(o.indexOf(this),1);}),document.addEventListener("contextmenu",function(t){o.forEach(function(e){t.target.matches(e.selector)&&e.show(t);});}),document.addEventListener("longtap",function(t){o.forEach(function(e){t.target.matches(e.selector)&&e.show(t);});}),document.addEventListener("click",function(t){o.forEach(function(e){t.target.matches('[data-contextmenu="'+
e.id+'"], [data-contextmenu="'+
e.id+'"] *')||e.hide();});}),a);});

/* Filename: 2_dragdrop.js */
if(doesExist("#drop-area")){const dropArea=document.getElementById("drop-area");["dragenter","dragover","dragleave","drop"].forEach((eventName)=>{dropArea.addEventListener(eventName,preventDefaults,false);document.body.addEventListener(eventName,preventDefaults,false);});dropArea.addEventListener("drop",handleDrop,false);function preventDefaults(e){e.preventDefault();e.stopPropagation();}
function highlight(e){dropArea.classList.add("highlight");}
function unhighlight(e){dropArea.classList.remove("active");}
function handleDrop(e){var dt=e.dataTransfer;var files=dt.files;let input=document.getElementById("documentFile");input.files=files;handleFiles(files);}
let uploadProgress=[];let progressBar=document.getElementById("progress-bar");function initializeProgress(numFiles){progressBar.value=0;uploadProgress=[];for(let i=numFiles;i>0;i--){uploadProgress.push(0);}}
function updateProgress(fileNumber,percent){uploadProgress[fileNumber]=percent;let total=uploadProgress.reduce((tot,curr)=>tot+curr,0)/
uploadProgress.length;progressBar.value=total;}
function handleFiles(files){files=[...files];files.forEach(uploadFile);files.forEach(prepareFiles);}
function prepareFiles(file){let reader=new FileReader();let fileName=document.getElementById("fileName");let fileContent=document.getElementById("fileContent");let prepare=document.getElementById("prepare");let select=prepare.querySelector("select");let submit=document.querySelector(".submitter");let drop=document.getElementById("drop");reader.readAsDataURL(file);reader.onloadend=function(){fileName.value=file.name.replace(/\.[a-zA-Z0-9]+$/,"");select.disabled=false;console.log(file);};}
...
for(var o in e)
n.d(r,o,function(t){return e[t];}.bind(null,o));return r;}),(n.n=function(e){var t=e&&e.__esModule?function(){return e.default;}:function(){return e;};return n.d(t,"a",t),t;}),(n.o=function(e,t){return Object.prototype.hasOwnProperty.call(e,t);}),(n.p="/"),n((n.s=5)));})([function(e,t,n){var r=n(1);"string"==typeof r&&(r=[[e.i,r,""]]);var o={hmr:!0,transform:void 0,insertInto:void 0};n(3)(r,o);r.locals&&(e.exports=r.locals);},function(e,t,n){(e.exports=n(2)(!1)).push([e.i,'.ce-delimiter {\n    line-height: 1.6em;\n    width: 100%;\n    text-align: center;\n}\n\n.ce-delimiter:before {\n    display: inline-block;\n    content: "***";\n    font-size: 30px;\n    line-height: 65px;\n    height: 30px;\n    letter-spacing: 0.2em;\n}',"",]);},function(e,t){e.exports=function(e){var t=[];return((t.toString=function(){return this.map(function(t){var n=(function(e,t){var n=e[1]||"",r=e[3];if(!r)return n;if(t&&"function"==typeof btoa){var o=((a=r),"/*# sourceMappingURL=data:application/json;charset=utf-8;base64,"+
btoa(unescape(encodeURIComponent(JSON.stringify(a))))+" */"),i=r.sources.map(function(e){return"/*# sourceURL="+r.sourceRoot+e+" */";});return[n].concat(i).concat([o]).join("\n");}
var a;return[n].join("\n");})(t,e);return t[2]?"@media "+t[2]+"{"+n+"}":n;}).join("");}),(t.i=function(e,n){"string"==typeof e&&(e=[[null,e,""]]);for(var r={},o=0;o<this.length;o++){var i=this[o][0];"number"==typeof i&&(r[i]=!0);}
for(o=0;o<e.length;o++){var a=e[o];("number"==typeof a[0]&&r[a[0]])||(n&&!a[2]?(a[2]=n):n&&(a[2]="("+a[2]+") and ("+n+")"),t.push(a));}}),t);};},function(e,t,n){var r,o,i={},a=((r=function(){return window&&document&&document.all&&!window.atob;}),function(){return void 0===o&&(o=r.apply(this,arguments)),o;}),s=(function(e){var t={};return function(e){if("function"==typeof e)return e();if(void 0===t[e]){var n=function(e){return document.querySelector(e);}.call(this,e);if(window.HTMLIFrameElement&&n instanceof window.HTMLIFrameElement)
try{n=n.contentDocument.head;}catch(e){n=null;}
t[e]=n;}
return t[e];};})(),u=null,c=0,f=[],l=n(4);function p(e,t){for(var n=0;n<e.length;n++){var r=e[n],o=i[r.id];if(o){o.refs++;for(var a=0;a<o.parts.length;a++)o.parts[a](r.parts[a]);for(;a<r.parts.length;a++)o.parts.push(m(r.parts[a],t));}else{var s=[];for(a=0;a<r.parts.length;a++)s.push(m(r.parts[a],t));i[r.id]={id:r.id,refs:1,parts:s};}}}
function d(e,t){for(var n=[],r={},o=0;o<e.length;o++){var i=e[o],a=t.base?i[0]+t.base:i[0],s={css:i[1],media:i[2],sourceMap:i[3]};r[a]?r[a].parts.push(s):n.push((r[a]={id:a,parts:[s]}));}
return n;}
function h(e,t){var n=s(e.insertInto);if(!n)
throw new Error("Couldn't find a style target. This probably means that the value for the 'insertInto' parameter is invalid.");var r=f[f.length-1];if("top"===e.insertAt)
r?r.nextSibling?n.insertBefore(t,r.nextSibling):n.appendChild(t):n.insertBefore(t,n.firstChild),f.push(t);else if("bottom"===e.insertAt)n.appendChild(t);else{if("object"!=typeof e.insertAt||!e.insertAt.before)
throw new Error("[Style Loader]\n\n Invalid value for parameter 'insertAt' ('options.insertAt') found.\n Must be 'top', 'bottom', or Object.\n (https://github.com/webpack-contrib/style-loader#insertat)\n");var o=s(e.insertInto+" "+e.insertAt.before);n.insertBefore(t,o);}}
function v(e){if(null===e.parentNode)return!1;e.parentNode.removeChild(e);var t=f.indexOf(e);t>=0&&f.splice(t,1);}
function b(e){var t=document.createElement("style");return(void 0===e.attrs.type&&(e.attrs.type="text/css"),y(t,e.attrs),h(e,t),t);}
function y(e,t){Object.keys(t).forEach(function(n){e.setAttribute(n,t[n]);});}
function m(e,t){var n,r,o,i;if(t.transform&&e.css){if(!(i=t.transform(e.css)))return function(){};e.css=i;}
if(t.singleton){var a=c++;(n=u||(u=b(t))),(r=x.bind(null,n,a,!1)),(o=x.bind(null,n,a,!0));}else
...
return((n.m=e),(n.c=t),(n.d=function(e,t,r){n.o(e,t)||Object.defineProperty(e,t,{enumerable:!0,get:r});}),(n.r=function(e){"undefined"!=typeof Symbol&&Symbol.toStringTag&&Object.defineProperty(e,Symbol.toStringTag,{value:"Module"}),Object.defineProperty(e,"__esModule",{value:!0});}),(n.t=function(e,t){if((1&t&&(e=n(e)),8&t))return e;if(4&t&&"object"==typeof e&&e&&e.__esModule)return e;var r=Object.create(null);if((n.r(r),Object.defineProperty(r,"default",{enumerable:!0,value:e}),2&t&&"string"!=typeof e))
for(var i in e)
n.d(r,i,function(t){return e[t];}.bind(null,i));return r;}),(n.n=function(e){var t=e&&e.__esModule?function(){return e.default;}:function(){return e;};return n.d(t,"a",t),t;}),(n.o=function(e,t){return Object.prototype.hasOwnProperty.call(e,t);}),(n.p="/"),n((n.s=4)));})([function(e,t,n){var r=n(1),i=n(2);"string"==typeof(i=i.__esModule?i.default:i)&&(i=[[e.i,i,""]]);var o={insert:"head",singleton:!1};r(i,o);e.exports=i.locals||{};},function(e,t,n){"use strict";var r,i=function(){return(void 0===r&&(r=Boolean(window&&document&&document.all&&!window.atob)),r);},o=(function(){var e={};return function(t){if(void 0===e[t]){var n=document.querySelector(t);if(window.HTMLIFrameElement&&n instanceof window.HTMLIFrameElement)
try{n=n.contentDocument.head;}catch(e){n=null;}
e[t]=n;}
return e[t];};})(),a=[];function s(e){for(var t=-1,n=0;n<a.length;n++)
if(a[n].identifier===e){t=n;break;}
return t;}
function c(e,t){for(var n={},r=[],i=0;i<e.length;i++){var o=e[i],c=t.base?o[0]+t.base:o[0],l=n[c]||0,u="".concat(c," ").concat(l);n[c]=l+1;var d=s(u),f={css:o[1],media:o[2],sourceMap:o[3]};-1!==d?(a[d].references++,a[d].updater(f)):a.push({identifier:u,updater:y(f,t),references:1}),r.push(u);}
return r;}
function l(e){var t=document.createElement("style"),r=e.attributes||{};if(void 0===r.nonce){var i=n.nc;i&&(r.nonce=i);}
...
return t;}
var u,d=((u=[]),function(e,t){return(u[e]=t),u.filter(Boolean).join("\n");});function f(e,t,n,r){var i=n?"":r.media?"@media ".concat(r.media," {").concat(r.css,"}"):r.css;if(e.styleSheet)e.styleSheet.cssText=d(t,i);else{var o=document.createTextNode(i),a=e.childNodes;a[t]&&e.removeChild(a[t]),a.length?e.insertBefore(o,a[t]):e.appendChild(o);}}
...
var h=null,m=0;function y(e,t){var n,r,i;if(t.singleton){var o=m++;(n=h||(h=l(t))),(r=f.bind(null,n,o,!1)),(i=f.bind(null,n,o,!0));}else
(n=l(t)),(r=p.bind(null,n,t)),(i=function(){!(function(e){if(null===e.parentNode)return!1;e.parentNode.removeChild(e);})(n);});return(r(e),function(t){if(t){if(t.css===e.css&&t.media===e.media&&t.sourceMap===e.sourceMap)
return;r((e=t));}else i();});}
e.exports=function(e,t){(t=t||{}).singleton||"boolean"==typeof t.singleton||(t.singleton=i());var n=c((e=e||[]),t);return function(e){if(((e=e||[]),"[object Array]"===Object.prototype.toString.call(e))){for(var r=0;r<n.length;r++){var i=s(n[r]);a[i].references--;}
for(var o=c(e,t),l=0;l<n.length;l++){var u=s(n[l]);0===a[u].references&&(a[u].updater(),a.splice(u,1));}
n=o;}};};},function(e,t,n){(t=n(3)(!1)).push([e.i,".cdx-list {\n    margin: 0;\n    padding-left: 40px;\n    outline: none;\n}\n\n    .cdx-list__item {\n        padding: 5.5px 0 5.5px 3px;\n        line-height: 1.6em;\n    }\n\n    .cdx-list--unordered {\n        list-style: disc;\n    }\n\n    .cdx-list--ordered {\n        list-style: decimal;\n    }\n\n    .cdx-list-settings {\n        display: flex;\n    }\n\n    .cdx-list-settings .cdx-settings-button {\n            width: 50%;\n        }\n","",]),(e.exports=t);},function(e,t,n){"use strict";e.exports=function(e){var t=[];return((t.toString=function(){return this.map(function(t){var n=(function(e,t){var n=e[1]||"",r=e[3];if(!r)return n;if(t&&"function"==typeof btoa){var i=((a=r),(s=btoa(unescape(encodeURIComponent(JSON.stringify(a))))),(c="sourceMappingURL=data:application/json;charset=utf-8;base64,".concat(s)),"/*# ".concat(c," */")),o=r.sources.map(function(e){return"/*# sourceURL=".concat(r.sourceRoot||"").concat(e," */");});return[n].concat(o).concat([i]).join("\n");}
var a,s,c;return[n].join("\n");})(t,e);return t[2]?"@media ".concat(t[2]," {").concat(n,"}"):n;}).join("");}),(t.i=function(e,n,r){"string"==typeof e&&(e=[[null,e,""]]);var i={};if(r)
for(var o=0;o<this.length;o++){var a=this[o][0];null!=a&&(i[a]=!0);}
for(var s=0;s<e.length;s++){var c=[].concat(e[s]);(r&&i[c[0]])||(n&&(c[2]?(c[2]="".concat(n," and ").concat(c[2])):(c[2]=n)),t.push(c));}}),t);};},function(e,t,n){"use strict";n.r(t),n.d(t,"default",function(){return d;});n(0);const r='<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><line x1="9" x2="19" y1="7" y2="7" stroke="currentColor" stroke-linecap="round" stroke-width="2"/><line x1="9" x2="19" y1="12" y2="12" stroke="currentColor" stroke-linecap="round" stroke-width="2"/><line x1="9" x2="19" y1="17" y2="17" stroke="currentColor" stroke-linecap="round" stroke-width="2"/><path stroke="currentColor" stroke-linecap="round" stroke-width="2" d="M5.00001 17H4.99002"/><path stroke="currentColor" stroke-linecap="round" stroke-width="2" d="M5.00001 12H4.99002"/><path stroke="currentColor" stroke-linecap="round" stroke-width="2" d="M5.00001 7H4.99002"/></svg>';function i(e){return((function(e){if(Array.isArray(e))return o(e);})(e)||(function(e){if("undefined"!=typeof Symbol&&Symbol.iterator in Object(e))
...
function a(e,t){var n=Object.keys(e);if(Object.getOwnPropertySymbols){var r=Object.getOwnPropertySymbols(e);t&&(r=r.filter(function(t){return Object.getOwnPropertyDescriptor(e,t).enumerable;})),n.push.apply(n,r);}
return n;}
...
/* Filename: 3_editor_table.js */
!(function(t,e){"object"==typeof exports&&"object"==typeof module?(module.exports=e()):"function"==typeof define&&define.amd?define([],e):"object"==typeof exports?(exports.Table=e()):(t.Table=e());})(window,function(){return(function(t){var e={};function o(r){if(e[r])return e[r].exports;var i=(e[r]={i:r,l:!1,exports:{}});return t[r].call(i.exports,i,i.exports,o),(i.l=!0),i.exports;}
...
return;r((t=e));}else i();});}
t.exports=function(t,e){if("undefined"!=typeof DEBUG&&DEBUG&&"object"!=typeof document)
throw new Error("The style-loader cannot be used in a non-browser environment");((e=e||{}).attrs="object"==typeof e.attrs?e.attrs:{}),e.singleton||"boolean"==typeof e.singleton||(e.singleton=s()),e.insertInto||(e.insertInto="head"),e.insertAt||(e.insertAt="bottom");var o=f(t,e);return(u(o,e),function(t){for(var r=[],i=0;i<o.length;i++){var s=o[i];(l=n[s.id]).refs--,r.push(l);}
t&&u(f(t,e),e);for(i=0;i<r.length;i++){var l;if(0===(l=r[i]).refs){for(var a=0;a<l.parts.length;a++)l.parts[a]();delete n[l.id];}}});};var x,C=((x=[]),function(t,e){return(x[t]=e),x.filter(Boolean).join("\n");});function y(t,e,o,r){var i=o?"":r.css;if(t.styleSheet)t.styleSheet.cssText=C(e,i);else{var n=document.createTextNode(i),s=t.childNodes;s[e]&&t.removeChild(s[e]),s.length?t.insertBefore(n,s[e]):t.appendChild(n);}}
function k(t,e){var o=e.css,r=e.media;if((r&&t.setAttribute("media",r),t.styleSheet))
t.styleSheet.cssText=o;else{for(;t.firstChild;)t.removeChild(t.firstChild);t.appendChild(document.createTextNode(o));}}
function R(t,e,o){var r=o.css,i=o.sourceMap,n=void 0===e.convertToAbsoluteUrls&&i;(e.convertToAbsoluteUrls||n)&&(r=p(r)),i&&(r+="\n/*# sourceMappingURL=data:application/json;base64,"+
btoa(unescape(encodeURIComponent(JSON.stringify(i))))+" */");var s=new Blob([r],{type:"text/css"}),l=t.href;(t.href=URL.createObjectURL(s)),l&&URL.revokeObjectURL(l);}},function(t,e){t.exports=function(t){var e="undefined"!=typeof window&&window.location;if(!e)throw new Error("fixUrls requires window.location");if(!t||"string"!=typeof t)return t;var o=e.protocol+"//"+e.host,r=o+e.pathname.replace(/\/[^\/]*$/,"/");return t.replace(/url\s*\(((?:[^)(]|\((?:[^)(]+|\([^)(]*\))*\))*)\)/gi,function(t,e){var i,n=e.trim().replace(/^"(.*)"$/,function(t,e){return e;}).replace(/^'(.*)'$/,function(t,e){return e;});return /^(#|data:|http:\/\/|https:\/\/|file:\/\/\/|\s*$)/i.test(n)?t:((i=0===n.indexOf("//")?n:0===n.indexOf("/")?o+n:r+n.replace(/^\.\//,"")),"url("+JSON.stringify(i)+")");});};},function(t,e,o){"use strict";function r(t,e,o={}){const r=document.createElement(t);Array.isArray(e)?r.classList.add(...e):e&&r.classList.add(e);for(const t in o)
Object.prototype.hasOwnProperty.call(o,t)&&(r[t]=o[t]);return r;}
function i(t){const e=t.getBoundingClientRect();return{y1:Math.floor(e.top+window.pageYOffset),x1:Math.floor(e.left+window.pageXOffset),x2:Math.floor(e.right+window.pageXOffset),y2:Math.floor(e.bottom+window.pageYOffset),};}
function n(t,e){const o=i(t),r=i(e);return{fromTopBorder:r.y1-o.y1,fromLeftBorder:r.x1-o.x1,fromRightBorder:o.x2-r.x2,fromBottomBorder:o.y2-r.y2,};}
function s(t,e){return e.parentNode.insertBefore(t,e);}
function l(t,e=!0){const o=document.createRange(),r=window.getSelection();o.selectNodeContents(t),o.collapse(e),r.removeAllRanges(),r.addRange(o);}
o.r(e);class a{constructor({items:t}){(this.items=t),(this.wrapper=void 0),(this.itemEls=[]);}
...
render(){return((this.wrapper=r("div",a.CSS.popover)),this.items.forEach((t,e)=>{const o=r("div",a.CSS.item),i=r("div",a.CSS.itemIcon,{innerHTML:t.icon}),n=r("div",a.CSS.itemLabel,{textContent:t.label});(o.dataset.index=e),o.appendChild(i),o.appendChild(n),this.wrapper.appendChild(o),this.itemEls.push(o);}),this.wrapper.addEventListener("click",(t)=>{this.popoverClicked(t);}),this.wrapper);}
popoverClicked(t){const e=t.target.closest("."+a.CSS.item);if(!e)return;const o=e.dataset.index,r=this.items[o];!r.confirmationRequired||this.hasConfirmationState(e)?r.onClick():this.setConfirmationState(e);}
...
open(){this.items.forEach((t,e)=>{"function"==typeof t.hideIf&&this.itemEls[e].classList.toggle(a.CSS.itemHidden,t.hideIf());}),this.wrapper.classList.add(a.CSS.popoverOpened);}
close(){this.wrapper.classList.remove(a.CSS.popoverOpened),this.itemEls.forEach((t)=>{this.clearConfirmationState(t);});}}
var c=o(0),d=o.n(c);class h{constructor({api:t,items:e,onOpen:o,onClose:r,cssModifier:i="",}){(this.api=t),(this.items=e),(this.onOpen=o),(this.onClose=r),(this.cssModifier=i),(this.popover=null),(this.wrapper=this.createToolbox());}
static get CSS(){return{toolbox:"tc-toolbox",toolboxShowed:"tc-toolbox--showed",toggler:"tc-toolbox__toggler",};}
get element(){return this.wrapper;}
createToolbox(){const t=r("div",[h.CSS.toolbox,this.cssModifier?`${h.CSS.toolbox}--${this.cssModifier}`:"",]),e=this.createPopover(),o=this.createToggler();return t.appendChild(o),t.appendChild(e),t;}
createToggler(){const t=r("div",h.CSS.toggler,{innerHTML:d.a});return(t.addEventListener("click",()=>{this.togglerClicked();}),t);}
...
show(t){const e=t();Object.entries(e).forEach(([t,e])=>{this.wrapper.style[t]=e;}),this.wrapper.classList.add(h.CSS.toolboxShowed);}
hide(){this.popover.close(),this.wrapper.classList.remove(h.CSS.toolboxShowed);}}
const p='<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-linecap="round" stroke-width="2" d="M8 8L12 12M12 12L16 16M12 12L16 8M12 12L8 16"/></svg>',u='<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-linecap="round" stroke-width="2" d="M12 7V12M12 17V12M17 12H12M12 12H7"/></svg>',f="tc-wrap",g="tc-wrap--readonly",m="tc-table",w="tc-row",b="tc-table--heading",v="tc-row--selected",x="tc-cell",C="tc-cell--selected",y="tc-add-row",k="tc-add-column";class R{constructor(t,e,o,r){(this.readOnly=t),(this.api=e),(this.data=o),(this.config=r),(this.wrapper=null),(this.table=null),(this.toolboxColumn=this.createColumnToolbox()),(this.toolboxRow=this.createRowToolbox()),this.createTableWrapper(),(this.hoveredRow=0),(this.hoveredColumn=0),(this.selectedRow=0),(this.selectedColumn=0),(this.tunes={withHeadings:!1}),this.resize(),this.fill(),(this.focusedCell={row:0,column:0}),(this.documentClicked=(t)=>{const e=null!==t.target.closest("."+m),o=null===t.target.closest("."+f);(e||o)&&this.hideToolboxes();const r=t.target.closest("."+y),i=t.target.closest("."+k);r&&r.parentNode===this.wrapper?(this.addRow(void 0,!0),this.hideToolboxes()):i&&i.parentNode===this.wrapper&&(this.addColumn(void 0,!0),this.hideToolboxes());}),this.readOnly||this.bindEvents();}
getWrapper(){return this.wrapper;}
bindEvents(){document.addEventListener("click",this.documentClicked),this.table.addEventListener("mousemove",(function(t,e){let o=0;return function(...r){const i=new Date().getTime();if(!(i-o<t))return(o=i),e(...r);};})(150,(t)=>this.onMouseMoveInTable(t)),{passive:!0}),(this.table.onkeypress=(t)=>this.onKeyPressListener(t)),this.table.addEventListener("keydown",(t)=>this.onKeyDownListener(t)),this.table.addEventListener("focusin",(t)=>this.focusInTableListener(t));}
createColumnToolbox(){return new h({api:this.api,cssModifier:"column",items:[{label:this.api.i18n.t("Add column to left"),icon:'<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.9167 14.9167L11.5833 18.25M11.5833 18.25L8.25 14.9167M11.5833 18.25L11.5833 10.0833C11.5833 9.19928 11.9345 8.35143 12.5596 7.72631C13.1848 7.10119 14.0326 6.75 14.9167 6.75"/></svg>',onClick:()=>{this.addColumn(this.selectedColumn,!0),this.hideToolboxes();},},{label:this.api.i18n.t("Add column to right"),icon:'<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.13333 14.9167L12.4667 18.25M12.4667 18.25L15.8 14.9167M12.4667 18.25L12.4667 10.0833C12.4667 9.19928 12.1155 8.35143 11.4904 7.72631C10.8652 7.10119 10.0174 6.75 9.13333 6.75"/></svg>',onClick:()=>{this.addColumn(this.selectedColumn+1,!0),this.hideToolboxes();},},{label:this.api.i18n.t("Delete column"),icon:p,hideIf:()=>1===this.numberOfColumns,confirmationRequired:!0,onClick:()=>{this.deleteColumn(this.selectedColumn),this.hideToolboxes();},},],onOpen:()=>{this.selectColumn(this.hoveredColumn),this.hideRowToolbox();},onClose:()=>{this.unselectColumn();},});}
createRowToolbox(){return new h({api:this.api,cssModifier:"row",items:[{label:this.api.i18n.t("Add row above"),icon:'<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.8833 15.8333L18.2167 12.5M18.2167 12.5L14.8833 9.16667M18.2167 12.5L10.05 12.5C9.16595 12.5 8.31811 12.8512 7.69299 13.4763C7.06787 14.1014 6.71667 14.9493 6.71667 15.8333"/></svg>',onClick:()=>{this.addRow(this.selectedRow,!0),this.hideToolboxes();},},{label:this.api.i18n.t("Add row below"),icon:'<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.8833 9.16666L18.2167 12.5M18.2167 12.5L14.8833 15.8333M18.2167 12.5H10.05C9.16594 12.5 8.31809 12.1488 7.69297 11.5237C7.06785 10.8986 6.71666 10.0507 6.71666 9.16666"/></svg>',onClick:()=>{this.addRow(this.selectedRow+1,!0),this.hideToolboxes();},},{label:this.api.i18n.t("Delete row"),icon:p,hideIf:()=>1===this.numberOfRows,confirmationRequired:!0,onClick:()=>{this.deleteRow(this.selectedRow),this.hideToolboxes();},},],onOpen:()=>{this.selectRow(this.hoveredRow),this.hideColumnToolbox();},onClose:()=>{this.unselectRow();},});}
moveCursorToNextRow(){this.focusedCell.row!==this.numberOfRows?((this.focusedCell.row+=1),this.focusCell(this.focusedCell)):(this.addRow(),(this.focusedCell.row+=1),this.focusCell(this.focusedCell),this.updateToolboxesPosition(0,0));}
getCell(t,e){return this.table.querySelector(`.${w}:nth-child(${t}).${x}:nth-child(${e})`);}
getRow(t){return this.table.querySelector(`.${w}:nth-child(${t})`);}
getRowByCell(t){return t.parentElement;}
getRowFirstCell(t){return t.querySelector(`.${x}:first-child`);}
setCellContent(t,e,o){this.getCell(t,e).innerHTML=o;}
addColumn(t=-1,e=!1){let o=this.numberOfColumns;for(let r=1;r<=this.numberOfRows;r++){let i;const n=this.createCell();if((t>0&&t<=o?((i=this.getCell(r,t)),s(n,i)):(i=this.getRow(r).appendChild(n)),1===r)){const i=this.getCell(r,t>0?t:o+1);i&&e&&l(i);}}
this.addHeadingAttrToFirstRow();}
addRow(t=-1,e=!1){let o,i=r("div",w);this.tunes.withHeadings&&this.removeHeadingAttrFromFirstRow();let n=this.numberOfColumns;if(t>0&&t<=this.numberOfRows){o=s(i,this.getRow(t));}else o=this.table.appendChild(i);this.fillRow(o,n),this.tunes.withHeadings&&this.addHeadingAttrToFirstRow();const a=this.getRowFirstCell(o);return a&&e&&l(a),o;}
deleteColumn(t){for(let e=1;e<=this.numberOfRows;e++){const o=this.getCell(e,t);if(!o)return;o.remove();}}
deleteRow(t){this.getRow(t).remove(),this.addHeadingAttrToFirstRow();}
createTableWrapper(){if(((this.wrapper=r("div",f)),(this.table=r("div",m)),this.readOnly&&this.wrapper.classList.add(g),this.wrapper.appendChild(this.toolboxRow.element),this.wrapper.appendChild(this.toolboxColumn.element),this.wrapper.appendChild(this.table),!this.readOnly)){const t=r("div",k,{innerHTML:u}),e=r("div",y,{innerHTML:u});this.wrapper.appendChild(t),this.wrapper.appendChild(e);}}
computeInitialSize(){const t=this.data&&this.data.content,e=Array.isArray(t),o=!!e&&t.length,r=e?t.length:void 0,i=o?t[0].length:void 0,n=Number.parseInt(this.config&&this.config.rows),s=Number.parseInt(this.config&&this.config.cols),l=!isNaN(n)&&n>0?n:void 0,a=!isNaN(s)&&s>0?s:void 0;return{rows:r||l||2,cols:i||a||2};}
resize(){const{rows:t,cols:e}=this.computeInitialSize();for(let e=0;e<t;e++)this.addRow();for(let t=0;t<e;t++)this.addColumn();}
fill(){const t=this.data;if(t&&t.content)
for(let e=0;e<t.content.length;e++)
for(let o=0;o<t.content[e].length;o++)
this.setCellContent(e+1,o+1,t.content[e][o]);}
fillRow(t,e){for(let o=1;o<=e;o++){const e=this.createCell();t.appendChild(e);}}
createCell(){return r("div",x,{contentEditable:!this.readOnly});}
get numberOfRows(){return this.table.childElementCount;}
get numberOfColumns(){return this.numberOfRows?this.table.querySelector(`.${w}:first-child`).childElementCount:0;}
get isColumnMenuShowing(){return 0!==this.selectedColumn;}
get isRowMenuShowing(){return 0!==this.selectedRow;}
...
onKeyPressListener(t){if("Enter"===t.key){if(t.shiftKey)return!0;this.moveCursorToNextRow();}
return"Enter"!==t.key;}
onKeyDownListener(t){"Tab"===t.key&&t.stopPropagation();}
focusInTableListener(t){const e=t.target,o=this.getRowByCell(e);this.focusedCell={row:Array.from(this.table.querySelectorAll("."+w)).indexOf(o)+1,column:Array.from(o.querySelectorAll("."+x)).indexOf(e)+1,};}
...
setHeadingsSetting(t){(this.tunes.withHeadings=t),t?(this.table.classList.add(b),this.addHeadingAttrToFirstRow()):(this.table.classList.remove(b),this.removeHeadingAttrFromFirstRow());}
addHeadingAttrToFirstRow(){for(let t=1;t<=this.numberOfColumns;t++){let e=this.getCell(1,t);e&&e.setAttribute("heading",this.api.i18n.t("Heading"));}}
removeHeadingAttrFromFirstRow(){for(let t=1;t<=this.numberOfColumns;t++){let e=this.getCell(1,t);e&&e.removeAttribute("heading");}}
selectRow(t){const e=this.getRow(t);e&&((this.selectedRow=t),e.classList.add(v));}
unselectRow(){if(this.selectedRow<=0)return;const t=this.table.querySelector("."+v);t&&t.classList.remove(v),(this.selectedRow=0);}
selectColumn(t){for(let e=1;e<=this.numberOfRows;e++){const o=this.getCell(e,t);o&&o.classList.add(C);}
this.selectedColumn=t;}
unselectColumn(){if(this.selectedColumn<=0)return;let t=this.table.querySelectorAll("."+C);Array.from(t).forEach((t)=>{t.classList.remove(C);}),(this.selectedColumn=0);}
getHoveredCell(t){let e=this.hoveredRow,o=this.hoveredColumn;const{width:r,height:i,x:n,y:s,}=(function(t,e){const o=t.getBoundingClientRect(),{width:r,height:i,x:n,y:s}=o,{clientX:l,clientY:a}=e;return{width:r,height:i,x:l-n,y:a-s};})(this.table,t);return(n>=0&&(o=this.binSearch(this.numberOfColumns,(t)=>this.getCell(1,t),({fromLeftBorder:t})=>n<t,({fromRightBorder:t})=>n>r-t)),s>=0&&(e=this.binSearch(this.numberOfRows,(t)=>this.getCell(t,1),({fromTopBorder:t})=>s<t,({fromBottomBorder:t})=>s>i-t)),{row:e||this.hoveredRow,column:o||this.hoveredColumn});}
binSearch(t,e,o,r){let i,s=0,l=t+1,a=0;for(;s<l-1&&a<10;){i=Math.ceil((s+l)/ 2);const t=e(i),c=n(this.table,t);if(o(c))l=i;else{if(!r(c))break;s=i;}
a++;}
return i;}
getData(){const t=[];for(let e=1;e<=this.numberOfRows;e++){const o=this.table.querySelector(`.${w}:nth-child(${e})`),r=Array.from(o.querySelectorAll("."+x));r.every((t)=>!t.textContent.trim())||t.push(r.map((t)=>t.innerHTML));}
return t;}
destroy(){document.removeEventListener("click",this.documentClicked);}}
o(1),(e.default=class{static get isReadOnlySupported(){return!0;}
static get enableLineBreaks(){return!0;}
constructor({data:t,config:e,api:o,readOnly:r}){(this.api=o),(this.readOnly=r),(this.config=e),(this.data={withHeadings:this.getConfig("withHeadings",!1,t),content:t&&t.content?t.content:[],}),(this.table=null);}
static get toolbox(){return{icon:'<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-width="2" d="M10 5V18.5"/><path stroke="currentColor" stroke-width="2" d="M5 10H19"/><rect width="14" height="14" x="5" y="5" stroke="currentColor" stroke-width="2" rx="4"/></svg>',title:"Table",};}
render(){return((this.table=new R(this.readOnly,this.api,this.data,this.config)),(this.container=r("div",this.api.styles.block)),this.container.appendChild(this.table.getWrapper()),this.table.setHeadingsSetting(this.data.withHeadings),this.container);}
...
save(){const t=this.table.getData();return{withHeadings:this.data.withHeadings,content:t};}
destroy(){this.table.destroy();}
getConfig(t,e,o){const r=this.data||o;return r?r[t]?r[t]:e:this.config&&this.config[t]?this.config[t]:e;}
static get pasteConfig(){return{tags:["TABLE","TR","TH","TD"]};}
onPaste(t){const e=t.detail.data,o=e.querySelector(":scope > thead, tr:first-of-type th"),r=Array.from(e.querySelectorAll("tr")).map((t)=>Array.from(t.querySelectorAll("th, td")).map((t)=>t.innerHTML));(this.data={withHeadings:null!==o,content:r}),this.table.wrapper&&this.table.wrapper.replaceWith(this.render());}});},]).default;});

/* Filename: scripts.js */
var contextImage=[{name:"Effacer image",fn:function(target){var targets=target.dataset.post.split(",");var xhr=new XMLHttpRequest();var formData=new FormData();formData.append("action","removeImage");formData.append("role",target.dataset.role);formData.append("iid",target.dataset.uri);xhr.open("POST",window.location.href,true);xhr.setRequestHeader("X-Requested-With","XMLHttpRequest");xhr.send(formData);xhr.onreadystatechange=function(){if(xhr.readyState===4){var response=createElementFromHTML(xhr.response);forEach(targets,function(index,value){var target=document.querySelector(value);var rTarget=response.querySelector(value);if(target&&rTarget){forEach(targets,function(index,value){tween({from:0,to:1,duration:550,onUpdate:(v)=>{target.style.opacity=v;},onComplete:(v)=>{target.style.opacity=null;},});});if(target.tagName=="INPUT"){target.value=rTarget.value;}else{target.innerHTML=rTarget.innerHTML;}}});if(xhr.status=="202"){setTimeout(function(){window.location.href=window.location.href;},2000);}}};},},];var contextAgenda=[{name:"Effacer Evènement",fn:function(target){var targets=target.dataset.post.split(",");var xhr=new XMLHttpRequest();var formData=new FormData();formData.append("action","supprimer");formData.append("eid",target.dataset.item);xhr.open("POST",window.location.href,true);xhr.setRequestHeader("X-Requested-With","XMLHttpRequest");xhr.send(formData);xhr.onreadystatechange=function(){if(xhr.readyState===4){var response=createElementFromHTML(xhr.response);forEach(targets,function(index,value){var target=document.querySelector(value);var rTarget=response.querySelector(value);if(target&&rTarget){forEach(targets,function(index,value){tween({from:0,to:1,duration:550,onUpdate:(v)=>{target.style.opacity=v;},onComplete:(v)=>{target.style.opacity=null;},});});if(target.tagName=="INPUT"){target.value=rTarget.value;}else{target.innerHTML=rTarget.innerHTML;}}});if(xhr.status=="202"){setTimeout(function(){window.location.href=window.location.href;},2000);}}};},},];var contextUser=[{name:"Retirer l'adhérent ",fn:function(target){var xhr=new XMLHttpRequest();var formData=new FormData();var targets=target.dataset.post.split(",");formData.append("action","removeUser");formData.append("uid",target.dataset.uid);formData.append('eid',target.dataset.eid);xhr.open("POST",target.dataset.action,true);xhr.setRequestHeader("X-Requested-With","XMLHttpRequest");xhr.send(formData);xhr.onreadystatechange=function(){if(xhr.readyState===4){var response=createElementFromHTML(xhr.response);forEach(targets,function(index,value){var target=document.querySelector(value);var rTarget=response.querySelector(value);if(target&&rTarget){forEach(targets,function(index,value){tween({from:0,to:1,duration:550,onUpdate:(v)=>{target.style.opacity=v;},onComplete:(v)=>{target.style.opacity=null;},});});if(target.tagName=="INPUT"){target.value=rTarget.value;}else{target.innerHTML=rTarget.innerHTML;}}});if(xhr.status=="202"){setTimeout(function(){window.location.href=window.location.href;},2000);}}};},},];var contextGroup=[{name:"Retirer",fn:function(target){var xhr=new XMLHttpRequest();var formData=new FormData();var targets=target.dataset.post.split(",");formData.append("action","remove2Group");formData.append("fid",target.dataset.group);formData.append('aid',target.dataset.aid);xhr.open("POST",target.dataset.action,true);xhr.setRequestHeader("X-Requested-With","XMLHttpRequest");xhr.send(formData);xhr.onreadystatechange=function(){if(xhr.readyState===4){var response=createElementFromHTML(xhr.response);forEach(targets,function(index,value){var target=document.querySelector(value);var rTarget=response.querySelector(value);if(target&&rTarget){forEach(targets,function(index,value){tween({from:0,to:1,duration:550,onUpdate:(v)=>{target.style.opacity=v;},onComplete:(v)=>{target.style.opacity=null;},});});if(target.tagName=="INPUT"){target.value=rTarget.value;}else{target.innerHTML=rTarget.innerHTML;}}});if(xhr.status=="202"){setTimeout(function(){window.location.href=window.location.href;},2000);}}};},},];var cm1=new ContextMenu("figure",contextImage);var cm2=new ContextMenu(".event",contextAgenda);var cm3=new ContextMenu(".status.user",contextUser);var cm4=new ContextMenu(".trctx",contextGroup);